                            ...
                            <div class="flip-card-inner">
                                <div class="flip-card-front">
                                    ...
                                    <div class="badge-keterangan">
                                        <div class="col-md-12">
                                            <span class="badge badge-success badge-diikuti badge-diikuti-{{ $m->id_mentor }}"><i class="fas fa-user-alt-slash"></i> Diikuti</span>
                                            <span class="badge badge-danger badge-penuh badge-penuh-{{ $m->id_mentor }}"><i class="fas fa-user-alt-slash"></i> Penuh</span>
                                            <span class="badge badge-info badge-ikuti-{{ $m->id_mentor }}" data-id="{{$m->id_mentor}}"><i class="fas fa-user-alt-slash"></i> Tidak mengikuti</span>
                                        </div>
                                    </div>
                                ...
                                <div class="flip-card-back rounded">
                                    <h4 class="font-weight-bold text-capitalize p-2">{{ $m->name }}</h4>
                                    <p>Mentor & Guru</p> 
                                    <p>{{ $m->email }}</p> 
                                    Jumlah Kuota :  <span class="kuota-kini-{{ $m->id_mentor }}"></span> / {{ $m->kuota }} <br>
                                    Status Kelas : <span class="status-penuh badge badge-danger status-penuh-{{ $m->id_mentor }}"> <i class="fas fa-user-alt-slash"></i> Penuh</span><span class="status-tersedia badge badge-success status-tersedia-{{ $m->id_mentor }}">Tersedia <i class="fas fa-check"></i></span><br>
                                    <br>
                                    <a class="btn btn-danger btn-sm btn-unfollow btn-unfollow-{{ $m->id_mentor }}" href="javascript:void(0)" data-id="{{ $m->id_mentor }}" data-nama="{{ $m->name }}"><i class="fa fa-sign-out-alt"></i> Berhenti ikuti mentor</a>
                                </div>
                                <form class="form-unfollow-{{ $m->id_mentor}}" action="{{ route("student.unfollow", $m->id_mentor) }}" method="POST">
                                    @csrf
                                </form>
                            </div>
                            <a class="btn-ikuti-{{ $m->id_mentor }} btn btn-info btn-ikuti shadow" style="width:90%;" href="{{ route('student.ikuti_mentor', $m->id_mentor) }}"><i class="fas fa-door-open"></i> Ikuti</a>
                            <button class="btn btn-danger shadow btn-penuh btn-penuh-{{ $m->id_mentor }}" type="button" style="width:90%;"><i class="fas fa-user-alt-slash"></i> Penuh</button>
                            <a class="btn-diikuti-{{ $m->id_mentor }} btn btn-success btn-diikuti shadow" style="width:90%;" href="javascript:void(0)"><i class="fas fa-map-pin"></i> Diikuti</a>
                        </div>
                    </div>
                @endforeach
            </div>
